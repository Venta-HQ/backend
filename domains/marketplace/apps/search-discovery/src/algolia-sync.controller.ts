...
export class AlgoliaSyncController implements OnModuleInit {
	...
	private async handleMarketplaceVendorEvent(data: {
		data:
			| EventDataMap['marketplace.vendor.onboarded']
			| EventDataMap['marketplace.vendor.profile_updated']
			| EventDataMap['marketplace.vendor.deactivated'];
		subject: string;
	}): Promise<void> {
	...
	private async handleLocationVendorEvent(data: {
		...
		try {
			...
			if (subject === 'location.vendor.location_updated') {
				await this.algoliaSyncService.updateVendorLocation(event);
